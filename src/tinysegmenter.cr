...
macro alias_class_method(old_method, new_method)
  def self.{{new_method.id}}(*args)
    self.{{old_method.id}}(*args)
  end
end
...
module TinySegmenter
  ...
  def self.get_tscore(dict, key)
    dict[key]? || 0
  end

  def self.get_char_type(str)
    REGEXP_TO_CHAR_TYPES.each do |regexp, chartype|
      return chartype if regexp =~ str
    end
    "O"
  end

  def self.get_char_type_with_cache(str)
    if @@cache.has_key? str
      @@cache[str]
    else
      @@cache[str] = get_char_type(str)
      @@cache[str]
    end
  end
  ...
  def self.tokenize(text)
    ...
    seg = ["B3","B2","B1"] of String
    char_type = ["O","O","O"]
    text.chars.each do |char|
      seg   << char.to_s
      char_type << get_char_type_with_cache(char.to_s)
    end
    seg.push("E1", "E2", "E3")
    char_type.push("O","O","O")

    word = seg[3]
    p1 = p2 = p3 = "U"
    (3..(seg.size-4)).each do |i|
      current_char = seg[i]
      score = @@_BIAS
      w1 = seg[i-3]
      w2 = seg[i-2]
      ...
      score += get_tscore(@@_UP1, p1)
      score += get_tscore(@@_UP2, p2)
      score += get_tscore(@@_UP3, p3)
      score += get_tscore(@@_BP1, p1 + p2)
      score += get_tscore(@@_BP2, p2 + p3)
      score += get_tscore(@@_UW1, w1)
      score += get_tscore(@@_UW2, w2)
      score += get_tscore(@@_UW3, w3)
      score += get_tscore(@@_UW4, w4)
      score += get_tscore(@@_UW5, w5)
      score += get_tscore(@@_UW6, w6)
      score += get_tscore(@@_BW1, w2 + w3)
      score += get_tscore(@@_BW2, w3 + w4)
      score += get_tscore(@@_BW3, w4 + w5)
      score += get_tscore(@@_TW1, w1 + w2 + w3)
      score += get_tscore(@@_TW2, w2 + w3 + w4)
      score += get_tscore(@@_TW3, w3 + w4 + w5)
      score += get_tscore(@@_TW4, w4 + w5 + w6)
      score += get_tscore(@@_UC1, c1)
      score += get_tscore(@@_UC2, c2)
      score += get_tscore(@@_UC3, c3)
      score += get_tscore(@@_UC4, c4)
      score += get_tscore(@@_UC5, c5)
      score += get_tscore(@@_UC6, c6)
      score += get_tscore(@@_BC1, c2 + c3)
      score += get_tscore(@@_BC2, c3 + c4)
      score += get_tscore(@@_BC3, c4 + c5)
      score += get_tscore(@@_TC1, c1 + c2 + c3)
      score += get_tscore(@@_TC2, c2 + c3 + c4)
      score += get_tscore(@@_TC3, c3 + c4 + c5)
      score += get_tscore(@@_TC4, c4 + c5 + c6)
      score += get_tscore(@@_UQ1, p1 + c1)
      score += get_tscore(@@_UQ2, p2 + c2)
      score += get_tscore(@@_UQ1, p3 + c3)
      score += get_tscore(@@_BQ1, p2 + c2 + c3)
      score += get_tscore(@@_BQ2, p2 + c3 + c4)
      score += get_tscore(@@_BQ3, p3 + c2 + c3)
      score += get_tscore(@@_BQ4, p3 + c3 + c4)
      score += get_tscore(@@_TQ1, p2 + c1 + c2 + c3)
      score += get_tscore(@@_TQ2, p2 + c2 + c3 + c4)
      score += get_tscore(@@_TQ3, p3 + c1 + c2 + c3)
      score += get_tscore(@@_TQ4, p3 + c2 + c3 + c4)
      p0 = "O"
      if score > 0
        result << word
        word = ""
